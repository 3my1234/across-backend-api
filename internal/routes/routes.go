...
func Register(app *fiber.App, db *pgxpool.Pool, cfg config.Config) {
	...
	adminRoutes.Get("/batches", opsOnly, admin.ListBatches)
	adminRoutes.Patch("/batches/:batch_id", opsOnly, admin.UpdateBatch)
	adminRoutes.Get("/batches/:batch_id/orders", opsOnly, admin.ListBatchOrders)
	...
	adminRoutes.Post("/batches/:batch_id/confirm-arrival", courierOnly, ops.ConfirmArrival)
